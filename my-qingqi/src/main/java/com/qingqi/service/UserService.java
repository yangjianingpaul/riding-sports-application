...
public class UserService {
     ...
     * @return
     */
    public PageResult queryNearUser(Double longitude, Double latitude, Double distance,
...
//        本月时间范围
        Date today = new Date();
        long miniTime = DateUtil.beginOfMonth(today).getTime();
...
//                    查询该用户本月的运动次数
                    Integer runCount = this.routeService.queryRouteCountByDate(nearUserVo.getUserId(), miniTime, maxTime);
    ...
    public Map<Long, User> queryUserMap(List<Object> userIdList){
        Query userId = Query.query(Criteria.where("userId").in(userIdList));
        return this.mongoTemplate.find(userId, User.class).stream().collect(Collectors.toMap(User::getUserId, user -> user));
    }
}
